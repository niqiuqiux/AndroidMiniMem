...
数据搜索、指针扫描、冻结、SO 注入、内置 AI 聊天（`gui/ai/`）、扫描/内存查看器/断点显示窗口均已删除。`client_singleton.h` 不再声明任何 Scan/Freeze 函数；新增能力请遵循"`socket/*Commands.cpp` 实现 → `client_singleton.h` 声明 → `IMemBackend`/`IMemService` 适配 → IPC handler / GUI / Lua 暴露"的链路。

## 约定

- 单例用 Meyer's（`GetInstance()`/`Get()`）。Socket 收发必须持端口锁；多命令业务操作还必须持 `TransactionLease`。任意 I/O 失败后连接进入 poisoned 状态，只能显式重连；普通同步 I/O 也必须保留有限默认超时。
- 顶层 Lua 执行必须通过 `LuaOperationBinding` 固定 `OperationContext`，Lua API 不得为每次内存操作重新抓取目标。
